...
ComfyUI 自定义节点，专为 **Higgs 语音合成** 设计。通过 4 个下拉框分别选择情感、风格、音效、韵律标签，自动输出完整格式的 Higgs 控制标签。
...
每个下拉框显示中文释义，选择后输出对应的完整格式标签。选中的标签以逗号拼接为一个字符串输出。
...
输出：
```
<|emotion:elation|>, <|style:singing|>, <|sfx:laughter|>
```
